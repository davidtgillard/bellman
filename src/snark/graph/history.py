...
def load_graph_history(root: Path) -> Result[GraphHistory, SnarkHistoryError]:
    ...
    Returns:
        ``Ok(GraphHistory)`` on success, or ``Err(SnarkHistoryError)`` when
    ...
    if not registry_path.is_file():
        return Err(
            SnarkHistoryError(
                "graph registry not found; run `snark init` or `snark validate --sync`",
