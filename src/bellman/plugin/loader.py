...
def list_plugins(root: Path) -> list[BellmanPlugin]:
...
def load_plugin(root: Path, name: str) -> BellmanPlugin | None:
    ...
    Returns:
        The registered :class:`BellmanPlugin`, or ``None`` when not found.
...
def load_plugin_or_raise(root: Path, name: str) -> BellmanPlugin:
