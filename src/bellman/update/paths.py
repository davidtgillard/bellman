"""Resolve bellman state and settings paths."""
...
BELLMAN_DIR = ".bellman"
STATE_FILENAME = "bellman-state.json"
SETTINGS_FILENAME = "bellman-settings.toml"


def running_executable() -> Path:
    """Path to the running bellman executable.

    ``sys.argv[0]`` is often a bare command name (for example ``bellman``) when
...
def executable_dir() -> Path:
    """Directory containing the bellman executable."""
    return running_executable().parent


def home_bellman_dir() -> Path:
    return Path.home() / BELLMAN_DIR


def local_bellman_dir() -> Path:
    return executable_dir() / BELLMAN_DIR


def settings_path() -> Path:
    """Settings always live under $HOME/.bellman/."""
    return home_bellman_dir() / SETTINGS_FILENAME


def state_read_path() -> Path | None:
    """Return existing state file path (local over home), or None."""
    local = local_bellman_dir() / STATE_FILENAME
    if local.is_file():
        return local
    home = home_bellman_dir() / STATE_FILENAME
    if home.is_file():
        return home
    return None
...
def state_write_path() -> Path:
    ...
    local_dir = local_bellman_dir()
    home_dir = home_bellman_dir()
...
def target_binary_path() -> Path:
    """Path to the running bellman binary."""
