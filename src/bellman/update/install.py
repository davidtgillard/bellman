"""Replace the running bellman binary with a downloaded build."""
...
def verify_update_permissions() -> Path:
    ...
    Returns:
        Resolved path to the running bellman binary.
    ...
    try:
        fd, name = tempfile.mkstemp(prefix=".bellman-perm-test-", dir=parent)
        os.close(fd)
        Path(name).unlink()
